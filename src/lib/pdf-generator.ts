...
function splitIntoChunks(content: string, targetCount: number): string[] {
  ...
  // If fewer chunks than target, split long paragraphs into sentence groups
  while (chunks.length < targetCount) {
    ...
    if (sentences.length <= 1) break; // can't split further
  ...
  // Pad empty pages if still not enough
...
export async function generateInteriorPdf(params: {
  ...
  const { childName, storyTitle, storyContent, theme } = params;
  ...
  for (let i = 0; i < STORY_PAGES; i++) {
    ...
    // Subtle decorative circle inside illustration area
    p.drawCircle({
  ...
  // ── Page 31: Fin de l'aventure ─────────────────────────────────────────────
  {
    ...
    drawCenteredText(p, 'FIN', bold, 42, PAGE_H * 0.68, accentColor);
    drawCenteredText(p, `Bravo ${childName} !`, bold, 18, PAGE_H * 0.54, white, PAGE_W, 0, 0.90);
    drawCenteredText(p, 'Tu as vécu une belle aventure.', regular, 14, PAGE_H * 0.46, white, PAGE_W, 0, 0.75);
    drawCenteredText(p, 'La prochaine attend déjà...', regular, 13, PAGE_H * 0.40, accentColor, PAGE_W, 0, 0.85);

    drawCenteredText(p, '✦ ✦ ✦', regular, 14, PAGE_H * 0.28, accentColor, PAGE_W, 0, 0.50);
    drawCenteredText(p, 'Kidshade', bold, 12, 28, accentColor, PAGE_W, 0, 0.60);
  }
  ...
  // ── Page 32: À propos ──────────────────────────────────────────────────────
  {
    ...
    drawCenteredText(p, 'À propos de Kidshade', bold, 17, PAGE_H - 68, rgb(ar * 0.7, ag * 0.7, ab * 0.7));
    ...
    for (const line of aboutLines) {
      ...
      drawCenteredText(p, line, isUrl ? bold : regular, isUrl ? 14 : 13, ay, isUrl ? accentColor : dark);
      ay -= 22;
    }
  }
...
export async function generateCoverPdf(params: {
  ...
  const { childName, storyTitle, theme } = params;
  const { bg, accent } = palette(theme);
  const [br, bg2, bb] = bg;
  const [ar, ag, ab] = accent;
  ...
  const backTexts: Array<{ text: string; size: number; bold: boolean }> = [
    ...
    { text: 'kidshade.com', size: 13, bold: true },
  ];
